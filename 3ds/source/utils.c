...
void get_swizzle_table(uint32_t *out, uint32_t w, uint32_t h) {
    static const uint8_t base8_unswizzle[64] = {
         0,  1,  4,  5, 16, 17, 20, 21,
         2,  3,  6,  7, 18, 19, 22, 23,
    ...
    uint32_t m = 0; // tile's offset: 0, 64, 128...

    for (uint32_t y = 0; y < h; y += 8) {
        for (uint32_t x = 0; x < w; x += 8) {

            for (uint32_t yy = 0; yy < 8 && (y + yy) < h; ++yy) {
                for (uint32_t xx = 0; xx < 8 && (x + xx) < w; ++xx) {
                    uint32_t dst = xy_to_i(x + xx, y + yy, w);
                    uint32_t in_tile = base8_unswizzle[yy * 8 + xx];
                    out[dst] = in_tile + m;
                }
            }
